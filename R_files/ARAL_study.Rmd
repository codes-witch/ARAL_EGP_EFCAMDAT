...
A total of 143 constructs do not show significant differences across levels.
```{r}
...
```{r}
# For all significant features,  do Wilcoxon Rank Sum tests to check for significant differences between the current level and the previous one. 
...
names(wcx_1side_pvals) <- c("feature", "A1", "A2", "B1", "B2", "C1")
...
for (f in 1:length(kw_sign_constructs)) {
  ...
  for (i in 1:(length(levels) - 1)){
    ...
    if (!appears_in_lev1 || !appears_in_lev2) {
      ...
      wcx_1side_result <- normalized_all_levels %>%
...
Based on the truth values, we predict the level of acquisition
```{r}
...
wcx_1side_level_predict <- fill_first_signif_pred(wcx_1side_pvals, wcx_1side_level_predict)

# fill in TRUE FALSE, is there a significant difference at the level the EGP says?
wcx_1side_level_predict <- fill_is_sig_at_lvl(wcx_1side_pvals, wcx_1side_level_predict)
```


```{r}
View(wcx_1side_level_predict)
```
